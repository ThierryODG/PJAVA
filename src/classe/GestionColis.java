...
public class GestionColis {
  ...
  public static void actualiserReception(DefaultTableModel tm){
    ...
    public static void modifier(String type_colis, double poids, String numero_envoi,String nomE,String prenomE,String telephoneE,String numero_pieceE,String type_pieceE,String villeE,
            String nomD,String prenomD,String telephoneD,String villeD,Date date_envoi,double coutEnvoi){
        try { 
            c=connexionbd.seconnecter();
            st=c.createStatement();
            st.executeUpdate("UPDATE colis SET poids ='"+poids+"', numero_envoi="+numero_envoi+",nomE="+nomE+",prenomE="+prenomE+",telephoneE="+telephoneE+",numero_pieceE="+numero_pieceE+",type_pieceE="+type_pieceE+","
                    + "villeE="+villeE+",nomD="+nomD+",prenomD="+prenomD+",telephoneD="+telephoneD+",villeD="+villeD+",date_envoi="+date_envoi+",coutEnvoi="+coutEnvoi+" WHERE type_colis='"+type_colis+"'");
    ...
    public static double getChiffreAffairesAnnuel(int annee) {
        ...
        try {
            Connection con = connexionbd.seconnecter();
            Statement st = con.createStatement();
            String query = "SELECT SUM(coutEnvoi) FROM colis WHERE YEAR(date_envoi) = " + annee;
            ResultSet rs = st.executeQuery(query);
            if (rs.next()) {
    ...
    try {
        Connection con = connexionbd.seconnecter();
        Statement st = con.createStatement();
        String query = "SELECT SUM(coutEnvoi) FROM colis WHERE YEAR(date_envoi) = " + annee + " AND MONTH(date_envoi) = " + mois;
        ResultSet rs = st.executeQuery(query);
        if (rs.next()) {
            totalCA = rs.getDouble(1);
        }
    ...
    public static boolean verifierMotDePasse(String motDePasse) {
        ...
        try {
            ...
            String query = "SELECT COUNT(*) FROM responsable WHERE mot_de_passer = ?";
            pst = con.prepareStatement(query);
            pst.setString(1, motDePasse);
            rs = pst.executeQuery();
            if (rs.next()) {
                estCorrect = rs.getInt(1) > 0;
            }
        } catch (SQLException | ClassNotFoundException ex) {
            ex.printStackTrace();
        } finally {
